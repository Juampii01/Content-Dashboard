...
// graph.facebook.com ig_exchange_token → profile via Graph API.
...
async function exchangeInstagram(
  ...
  // Long-lived tokens expire in ~60 days but are refreshable via ig_refresh_token.
  // We don't pre-set expiresAt — sync marks the token expired only on error 190.
