...
export function AnalizadorContent() {
  ...
  const handleSearch = useCallback(async () => {
    ...
    setSearched(true)
    setElapsed(0)
    setProgress(`Scrapeando los últimos ${SCRAPE_LIMIT} reels de @${handle}...`)

    if (timerRef.current) clearInterval(timerRef.current)
    timerRef.current = setInterval(() => setElapsed((s) => s + 1), 1000)

    try {
      const res = await fetch('/api/analizador/scrape', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username: handle, limit: SCRAPE_LIMIT }),
      ...
      setProgress(`Seleccionando los ${TOP_N} mejores por vistas...`)
      const data = await res.json()
      const all: ScrapedReel[] = data.reels ?? []
      // Pick top N by views, then by likes as tiebreaker
      const top = [...all]
        .sort((a, b) => (b.videoPlayCount ?? 0) - (a.videoPlayCount ?? 0) || (b.likesCount ?? 0) - (a.likesCount ?? 0))
        .slice(0, TOP_N)
      setReels(top)
      setCacheData({ username: handle, limit: SCRAPE_LIMIT, reels: top, cachedAt: Date.now() })
      setProgress('')
    } catch (e) {
    ...
    <div className="page-shell">
      ...
      {reels.length > 0 && (
        <div className="flex items-center justify-between mb-4">
          <p className="text-sm font-medium" style={{ color: 'var(--foreground)' }}>
            Top {reels.length} por vistas · últimos {SCRAPE_LIMIT} reels de @{cacheData.username}
          </p>
        </div>
      )}
      ...
      {reels.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {reels.map((reel, index) => (
            ...
            >
              <ReelAnalyzerCard reel={reel} rank={index + 1} />
            </div>
          ))}
        </div>
      )}
      ...
      {!searched && (
        <div className="text-center py-16" style={{ color: 'var(--muted-foreground)' }}>
          ...
          <p className="text-xs mt-1 opacity-70">Scrapeamos los últimos {SCRAPE_LIMIT} reels y mostramos los {TOP_N} con más vistas</p>
        </div>
      )}
    </div>
  )
