...
function usePublishHistory() {
  ...
  const reload = () => {
    ...
    fetch('/api/instagram/publish')
      ...
      .then(d => { setPosts((d.posts ?? []) as Post[]); setLoading(false) })
  ...
  async function publish() {
    ...
    try {
      const res = await fetch('/api/instagram/publish', {
        ...
        body: JSON.stringify({ mediaType, mediaUrl: mediaUrl.trim(), caption: caption.trim() || null }),
      })
    ...
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] overflow-hidden">
        <div className="px-5 pt-5 pb-5">
          ...
          <div className="flex gap-2 mb-4">
            {(['IMAGE', 'REEL'] as const).map(type => (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${mediaType === type ? IG_GRADIENT_CSS + ' text-white' : 'bg-[var(--muted)] text-[var(--muted-foreground)] hover:text-[var(--foreground)]'}`}
              >
          ...
          <div className="mb-4">
            ...
            <div className="h-1.5 bg-[var(--muted)] rounded-full overflow-hidden">
              <div
                ...
                style={{ width: quotaPct + '%', background: quotaPct > 80 ? '#FD1D1D' : IG_GRADIENT }}
              />
            </div>
          </div>
          ...
          >
            {publishing
              ? <><Loader2 size={16} className="animate-spin" /> Publicando{mediaType === 'REEL' ? ' (puede tardar hasta 30s)' : ''}...</>
            ...
            {posts.slice(0, 10).map(post => (
              <div key={post.id} className="flex items-center gap-3 px-5 py-3.5">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${
                  post.status === 'PUBLISHED' ? 'bg-emerald-500/20'
